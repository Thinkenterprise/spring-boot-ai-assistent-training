...
Die Modellkonfiguration erfolgt über `application.yaml`. Über **ChatOptions** werden Parameter wie `temperature` und `num-predict` gesteuert:

| Parameter | Beschreibung |
|---|---|
| `temperature` | Zufälligkeit der Antwort (0.0 = deterministisch, 1.0 = kreativ) |
| `num-predict` | Maximale Länge der generierten Antwort (Output) |
| `num-ctx` | Größe des Kontextfensters (maximale Token für Input + Output zusammen) |

> **Wichtig:** `num-predict` sollte immer kleiner oder gleich `num-ctx` sein, da `num-predict` nur die Antwort begrenzt, während `num-ctx` das gesamte Kontextfenster (Eingabe + Ausgabe) definiert. Im Beispiel (num-predict=512, num-ctx=2048) kann die Antwort maximal 512 Token lang sein, während das Modell insgesamt bis zu 2048 Token im Kontext verarbeiten kann. 
...
> **Hinweis:** `spring.profiles.active: init` aktiviert das Profil `init` und damit die
> `@Profile("init")`-annotierte `ApplicationRunner`-Bean — unabhängig davon, ob die
> Anwendung über `mvn spring-boot:run` oder aus der IDE gestartet wird.


## Test

1. Anwendung starten: `mvn spring-boot:run`
2. Es wird auf der Konsole die Antwort auf "Hello" ausgegeben.  
