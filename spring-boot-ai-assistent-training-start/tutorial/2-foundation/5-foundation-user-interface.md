...
In diesem Schritt haben wir die statische Web-Seite generieren lassen und nicht von Grund auf selbst implementiert. Sie liegt unter `src/main/resources/static/index.html` und wird von Spring Boot als Willkommensseite automatisch unter `/` ausgeliefert.
...
> **Hinweis:** `chat.html` ist in dieser Session nicht relevant — die Datei enthält lediglich den Platzhaltertext „Please Implement a Chat UI" und gehört inhaltlich eher zum Hinweistext aus Session 2.1.

Die statische HTML-Seite kommuniziert mit einem REST-Endpunkt im Backend, um die Chat-Anfragen zu verarbeiten. Der Controller reicht die Anfrage an den Service weiter und gibt die Antwort des AI-Modells zurück. Eine Conversation-ID wird an dieser Stelle noch nicht verwendet — das kommt erst in der Einheit zu Chat Memory hinzu:

```java
@PostMapping
public Map<String, String> chat(@RequestBody Map<String, String> payload) {
    String message = payload.get("message");
    var result = insuranceChatService.chatServiceWithoutMemory(message);
